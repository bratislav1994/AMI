...
namespace FillingScript
{
    class Program
    {
        static Dictionary<int, float> householdConsumption = new Dictionary<int, float>();
        static Dictionary<int, float> shoppingCenterConsumption = new Dictionary<int, float>();
        static Dictionary<int, float> firmConsumption = new Dictionary<int, float>();
        ...
        static void Main(string[] args)
        {
            ...
            InitHousehold();
            InitShoppingCenter();
            List<DynamicMeasurement> measurementsList = new List<DynamicMeasurement>();
            ...
            ChannelFactory<ICalculationEngineForScript> factoryCE = new ChannelFactory<ICalculationEngineForScript>(
                binding,
                new EndpointAddress("net.tcp://localhost:10010/ICalculationEngineForScript/FillingScript"));
            ICalculationEngineForScript proxyCE = factoryCE.CreateChannel();
            ...
            int current = 0;
            bool finished = false;
            while (current <= measurementsList.Count)
            {
                if(count > measurementsList.Count - current)
                {
                    count = measurementsList.Count - current;
                    finished = true;
                }
                proxyCE.FillDataBase(measurementsList.GetRange(startRange, count));
                startRange += count;
                current += count;
                if(finished)
                {
